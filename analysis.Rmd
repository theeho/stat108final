...
An important note is that we cannot assume all human perceptible musical features are dependent on genre. 
...
We can observe that some paramaters have a high p value particularly ones that are ascociated with key. This may be an example of an audio feature that does not determine the genre of a song. Based on the context of the problem, this is not unreasonable because it is difficult for humans to differ a tracks genre based on its key. 
...
Aside from the key, another alarmingly large p-value is the tempo. Particularly, in country and rock. Other response categories have a much lower p-value for tempo, however confidence interval is close to 0. 
...
```
```{r}
full_model_key <- multinom(genre ~ danceability + energy + loudness + speechiness + acousticness + instrumentalness + liveness + valence + tempo + duration_sec + key, data = g1)

red_model <- multinom(genre ~ danceability + energy + loudness + speechiness  + instrumentalness + liveness + valence + tempo  + acousticness, data = g1)
